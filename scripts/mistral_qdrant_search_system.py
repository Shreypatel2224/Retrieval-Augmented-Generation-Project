...
embedding_models = {
    ...
    "all-mpnet-base-v2": SentenceTransformer("all-mpnet-base-v2"),  # 768 dims
    "paraphrase-MiniLM-L6-v2": SentenceTransformer("paraphrase-MiniLM-L6-v2"),  # 384 dims
}
...
def create_qdrant_collection(model_name):
    vector_size = len(embedding_models[model_name].encode("test"))
    collection_name = get_collection_name(model_name)
...
def search_embeddings(query, model_name, top_k=3):
    collection_name = get_collection_name(model_name)
    query_vector = get_embedding(query, model_name)
    results = qdrant.search(collection_name=collection_name, query_vector=query_vector, limit=top_k)
    return [
        {
...
def generate_rag_response(query, context_results):
    context_str = "\n".join(
        [f"From model {r['model']}, embedding ID {r['embedding_id']} with similarity {r['similarity']:.2f}" for r in context_results]
    )
...
def log_performance_to_csv(embedding_folder, database, llm, model_name, query, search_time, response_time, memory_mb, response):
    ...
    with open(csv_file, mode="a", newline="", encoding="utf-8") as file:
        ...
        if not file_exists:
            writer.writerow(["Embedding Folder", "Database", "LLM", "Embedding Model", "Query", "Search Time (seconds)", "Response Time (seconds)", "Memory (MB)", "Response"])
        writer.writerow([embedding_folder, database, llm, model_name, query, search_time, response_time, memory_mb, response])
...
def interactive_search(embedding_base_folder):
    ...
    while True:
        ...
        log_performance_to_csv(
            embedding_base_folder, "Qdrant", "Mistral", model_name, query,
